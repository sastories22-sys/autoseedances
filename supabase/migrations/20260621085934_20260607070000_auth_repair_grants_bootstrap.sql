...
- Table-level CRUD grants for all core tables
- has_role() and ensure_user_records() grants to authenticated, service_role

2. New Functions
- ensure_user_records(user_id, email, raw_meta): idempotent bootstrap for all user tables
- ensure_user_bootstrap(): user-callable wrapper using auth.uid()
- handle_new_user(): trigger-safe wrapper for signup

3. RLS Policy Refresh
- All policies re-created with idempotent DROP/CREATE
- Admins get override via has_role() everywhere

4. Backfill
- All existing auth.users get bootstrapped with records
*/
