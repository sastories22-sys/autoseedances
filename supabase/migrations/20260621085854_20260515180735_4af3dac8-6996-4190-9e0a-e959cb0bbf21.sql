/*
# Initial schema: roles, profiles, subscriptions, prompts, queue, files, usage, settings

1. New Tables
- user_roles (user_id, role app_role enum)
- profiles (id, display_name, avatar_url, extension_connected, extension_last_seen)
- subscriptions (user_id, plan, status, stripe_customer_id, stripe_subscription_id, current_period_end)
- prompts (user_id, text, tags, platform)
- queue_jobs (user_id, prompt_text, platform, status, progress, error, started_at, finished_at)
- generated_files (user_id, job_id, url, thumbnail_url, duration_seconds, prompt_text, platform)
- usage_tracking (user_id, day, prompts_used)
- user_settings (user_id, theme, automation_speed, delay_ms, auto_download, download_path, default_platform)
...
3. New Functions
- has_role(user_id, role) for admin checks
- handle_new_user() trigger for bootstrap on signup
